...
class RemoteB:
    ...
    def __init__(self, uri='http://localhost:8270', *args):
        if '://' not in uri:
            uri = 'http://' + uri
        self._client = XmlRpcRemoteClient(uri,*args)
...
class XmlRpcRemoteClient:
    ...
    def __init__(self, uri, *args):
        ...
        for library in args:
            self.remote_import(library, self._export(library))

    def _export(self, library):
        exec('import %s as m' % library)
        filename = m.__file__[:-1] #make pyc = py
        return open(filename).read()

    def remote_import(self, lib_name, code):
        try:
            return self._server.remote_import(lib_name, code)
...
if __name__ == '__main__':
    r = Remote('http://localhost:8270','OperatingSystem')
    r.run_keyword('create_file', ['foobar'])
